...
//      cout << "Max_Projection " << Max_Projection << " BinMin_Projection " << BinMin_Projection << " BinMax_Projection " << BinMax_Projection << " RangeMin " << RangeMin << " RangeMax " << RangeMax<< endl;
 ...
 void Analyse_Multi_Ekin_XY_SetupArronax(){
     ...
     if(Farm == true){
         vec_Option.clear();
         vec_Option.push_back("vDefault_68MeV_0_");
         //          vec_Option.push_back("vDefault_68MeV_0.68_");
         //          vec_Option.push_back("vDefault_68MeV_1_");
         ...
         vec_Option.clear();
         vec_Option.push_back("vDefault2_Ligne_ICO_v1T_68MeV_0_");
...
//                          vec_Option.clear();
//          NameSetup="vACDv4.3_Ligne_ICO_v2WT_BIC_G4_WATER";
//          CompleteNameSetup= NameSetup +"_68MeV_0_" ; vec_Option.push_back(CompleteNameSetup);
...
//       vec_TCut.push_back("ParentID!=0");
     ...
     fichier_Save2 << "NameCase_TS x_TCut Projection Range_Min Range_Max Amplitude_Min Amplitude_Max Mean STD Homogeneite_1 Homogeneite_2" << endl;
     
     if(PlotCaseByCase==true){
     
         for(auto& x_TCut: vec_TCut){
         
//              fichier_Save2 << x_TCut << endl;
...
//              float Max_Projection = Projection_X->GetMaximum();
//              
//              int BinMin_Projection = (Float_t)Projection_X->FindFirstBinAbove(Max_Projection*0.8) ;
//              int BinMax_Projection = (Float_t)Projection_X->FindLastBinAbove(Max_Projection*0.8) ;
//              
//              float RangeMin = Projection_X->GetBinCenter(BinMin_Projection);
//              float RangeMax = Projection_X->GetBinCenter(BinMax_Projection);
//              
//              vector<float> vec_Projection;
//             for(int i = BinMin_Projection ; i< BinMax_Projection; i++){
//                   vec_Projection.push_back(Projection_X->GetBinContent(i));
//             }
//              std::sort (vec_Projection.begin(), vec_Projection.end(), myfunction_float);
//              
//              
...
//              
//             vec_Projection.clear();
//              BinMin_Projection = Projection_X->FindBin(-1*(RangeMax * 0.80));
//             BinMax_Projection = Projection_X->FindBin(1*(RangeMax * 0.80));
//             for(int i = BinMin_Projection ; i< BinMax_Projection; i++){
//                   vec_Projection.push_back(Projection_X->GetBinContent(i));
//             }
//              std::sort (vec_Projection.begin(), vec_Projection.end(), myfunction_float);
...
//              cout << "Max_Projection " << Max_Projection << " BinMin_Projection " << BinMin_Projection << " BinMax_Projection " << BinMax_Projection << " RangeMin " << RangeMin << " RangeMax " << RangeMax<< endl;
             ...
             TH1D* Projection_Y = (TH1D*)h_graph2->ProjectionY("ProjectionY of binx=[1996,2006] [x=-0.55..0.55]",1986,2016)->Clone();
             //              h_graph =  Projection_X;
...
//              map  <int, map < TString, float>> map_Results;
     ...
     ofstream fichier_Save(Form("%s%s",Total_Path.Data(),"Results.txt"), ios::out | ios::trunc);
         ...
         fichier_Save <<"\n*** i_Config " << i_Config << " " << vec_Option[i_Config] << endl; 
        ...
        int i_hgr = -1;
         ...
         Max = const_ICO->fMax;              
         TH1F *h_d_Clone = (TH1F*)hgr->Clone("h_d_Clone");
         
         if(i_hgr==0){
         
         NameCanvas = constants->NameCanvas(Number_temp, vec_Option[i_Config]+ NameCase_TS + h_d_Clone->GetTitle() );
         C_temp = new TCanvas(NameCanvas, NameCanvas,0,25,1920,1041);
