 ...
 void Analyse_Dose_Souris_v5_distribute(){
     ...
     if(Farm == true){
         ...
         vec_Option.clear();
         NameSetup="vSourisv4.2_Ligne_ICO_v2T";
         CompleteNameSetup= NameSetup +"_68MeV_0_" ; vec_Option.push_back(CompleteNameSetup);
         CompleteNameSetup= NameSetup +"_68MeV_0.68_" ; vec_Option.push_back(CompleteNameSetup);
         CompleteNameSetup= NameSetup +"_68MeV_1_" ; vec_Option.push_back(CompleteNameSetup);
         
         vec_Option.clear();
         NameSetup="vSv5.0_Ligne_ICO_v2T_BIC_Crop_v1_G4_WATER";
...
//          CompleteNameSetup= NameSetup +"_68MeV_1" ; vec_Option.push_back(CompleteNameSetup);
                           ...
                           vec_Option.clear();
         NameSetup="vSv5.2_Ligne_ICO_v2T_BIC_Crop_v1_G4_WATER";
         CompleteNameSetup= NameSetup +"_68MeV_0" ; vec_Option.push_back(CompleteNameSetup);
         
         
//          output_Ref_CT2_vSv5.0_Ligne_ICO_v2T_BIC_Crop_v1_G4_WATER_68MeV_0_10M
//         Distribute_Ref_CT2_vSv5.0_Ligne_ICO_v2T_BIC_Crop_v1_G4_WATER_68MeV_0
     ...
     std::vector<string> Category_Of_Analyse {"CT_GridSize", "CT2_ImagingCalibration","Pos"};
     std::vector<string> type_Of_Analyse_2 {"1", "2"};
     
           Category_Of_Analyse.clear(); 
//       type_Of_Analyse_2.clear();
     Category_Of_Analyse.push_back("I");
     
     
     std::vector<string> vec_Ref_Case ;
     
     for(auto& x: vec_Option){
         string Title = "Distribute_Ref_CT2_" + x;
...
//          dose_Cube_after_All
         ...
         NameInput = PathInput + "/souris-Dose"+ TS_DoseToWater +".root";
         ...
         NameInput = PathInput + "/souris-DoseToWater.root";
         ...
         NameInput = PathInput + "/souris-Dose"+ + TS_DoseToWater + "-Uncertainty.root";
         ...
         NameInput = PathInput + "/souris-NbOfHits.root";
         ...
         NameInput = PathInput + "/souris-Dose" +TS_DoseToWater +"-Squared.root";
...
//          
         ...
         N_Primaries = ToDouble(map_Comment_Ref[i_Setup]["/gate/application/setTotalNumberOfPrimaries"].Data());
                  ...
                  ifstream fichier (Form("%s/%s",PathInput.Data(),"stat.ascii"),ios::in);// ouvre un nouveau fichier en lecture. On lui associe l'objet: fichier
         if(!fichier){cout << " L'erreur est humaine, admettre la sienne est surhumain. [Doug Larson] stat ascii " << endl;}
         else{
             
             //              # NumberOfEvents = 29999999
             
             while (fichier){
                 
                 string s;
                 if (!getline( fichier, s )) break;
                 istringstream ss( s );TString Line = s;
                 if(Line.Contains("NumberOfEvents")){
                 ...
                 N_Primaries= ToDouble(v_record[v_record.size()-1]);
                 map_Comment_Ref[i_Setup]["/gate/application/setTotalNumberOfPrimaries"]=v_record[v_record.size()-1];
                 break;
                 }
...
//                      {"CT_GridSize", "CT2_ImagingCalibration"};
                     
                     if(x == "CT_GridSize"){
                     
                         if(y == "1")AnalyseFile= "CT04mm";
                        else if(y == "2") AnalyseFile= "CT08mm";
                        else {cout << "unknow CT_GridSize" << endl; }
                         
                    }
                    else if (x == "CT2_ImagingCalibration"){
                        if(y == "1")AnalyseFile= "CT2m40HU";
                        else if(y == "2") AnalyseFile= "CT2p40HU";
                        else {cout << "unknow CT2_ImagingCalibration" << endl; }
                    }
                    else if (x == "Pos"){
                        if(y == "1")AnalyseFile= "Posm1cm";
                        else if(y == "2") AnalyseFile= "Posp1cm";
                        else {cout << "unknow CT2_ImagingCalibration" << endl; }
                    }
                    else if (x == "I"){
                        if(y == "1")AnalyseFile= "I_Min";
                        else if(y == "2") AnalyseFile= "I_Max";
                        else {cout << "unknow I-VALUE" << endl; }
                    }
                    else{
                        cout << "Unknow Case Exit 0" << endl; exit(0);
                    }
                     
                     string PathFile_Total = PathFile + "Distribute_"+ AnalyseFile.Data() + "_" + vec_Option[i]  + "/";
                     
                     
                     
                     NameCase = "Distribute_"+ AnalyseFile + "_" + vec_Option[i];
                     PathInput =  PathFile + NameCase;
                     
                     cout << "Path Input " << PathInput << endl;
                     
                     NameInput = PathInput + "/souris-Dose.root";
                     myFile_Ref_Dose = TFile::Open(NameInput.Data()); 
                     h_dose = (TH1D*)myFile_Ref_Dose->Get("histo");  
                     h_dose_Clone = (TH1D*)h_dose->Clone();
                     
                     
                     NameInput = PathInput + "/souris-Dose-Uncertainty.root";
                     ...
                     NameInput = PathInput + "/souris-Dose-Squared.root";
...
//                      }
                     ...
                     map_Histo_StudyCase[i_Setup][x][y]["hdose"] = h_dose_Clone;
                     map_Histo_StudyCase[i_Setup][x][y]["hdose_Uncertainty"] = h_dose_Uncertainty_Clone;
                     map_Histo_StudyCase[i_Setup][x][y]["h_dose_NbOfHits"] = h_dose_NbOfHits_Clone;
...
//    leg->SetFillStyle(1001);
     ...
     TLegend *leg2 = new TLegend(0.1872385,0.5019763,0.540795,0.6442688,NULL,"brNDC");
...
//          Data += " FWHM " + strObj2;
         ...
         if(WichCase.Contains("_0.68")) { 
             
             leg2->AddEntry(x.second["hdose"], "68 #pm 0.68 MeV " + Data, "l");
             
         }
         if(WichCase.Contains("_1"))  leg2->AddEntry(x.second["hdose"], "68 #pm 1.00 MeV " + Data, "l");
         if(WichCase.Contains("_0")) { leg2->AddEntry(x.second["hdose"], "68 #pm 0.00 MeV " + Data, "l");
...
//    leg->SetFillStyle(1001);
      ...
      leg2 =new TLegend(0.1966527,0.5227273,0.3671548,0.6650198,NULL,"brNDC");
     leg2->SetBorderSize(1);
     leg2->SetTextAlign(32);
     leg2->SetTextSize(0.0403071);
     leg2->SetLineColor(1);
...
//          Data += " FWHM " + strObj2;
         ...
         if(WichCase.Contains("_1"))  leg2->AddEntry(x.second["hdoseToWater"], "68 #pm 1.00 MeV " + Data, "l");
         if(WichCase.Contains("_0")) { leg2->AddEntry(x.second["hdoseToWater"], "68 #pm 0.00 MeV " + Data, "l");
...
//**********************************************************************************************************************************************     
     ...
     for(auto& x: map_Histo_Ref){
       ...
       if(Max_Y < h_Clone->GetMaximum())     Max_Y = h_Clone->GetMaximum();
     ...
     for(auto& x: map_Histo_Ref){
         ...
         if(WichCase.Contains("_0.68"))  leg2->AddEntry(x.second["hdose"], "68 #pm 0.68 MeV", "l");
         if(WichCase.Contains("_1"))  leg2->AddEntry(x.second["hdose"], "68 #pm 1.00 MeV", "l");
         if(WichCase.Contains("_0"))  leg2->AddEntry(x.second["hdose"], "68 #pm 0.00 MeV", "l");
         ...
         h_Clone->GetYaxis()->SetRangeUser(0,Max_Y);
         ...
         h_Clone_Zoom->GetXaxis()->SetRangeUser(-10,30);
...
//**********************************************************************************************************************************************     
     ...
     C_temp= new TCanvas(NameCanvas, NameCanvas,0,25,1920,1041);
          ...
          leg2 = new TLegend(0.6762552,0.479249,0.8634937,0.6541502,NULL,"brNDC");
     leg2->SetBorderSize(1);
     leg2->SetTextAlign(32);
     leg2->SetTextSize(0.0403071);
     leg2->SetLineColor(1);
     ...
     TLegend *leg22 = new TLegend(0.6762552,0.479249,0.8634937,0.6541502,NULL,"brNDC");
     leg22->SetBorderSize(1);
     leg22->SetTextAlign(32);
     leg22->SetTextSize(0.0403071);
     leg22->SetLineColor(1);
     leg22->SetLineStyle(1);
     leg22->SetLineWidth(2);
     ...
     for(auto& x: map_Histo_Ref){
         ...
         if(WichCase.Contains("_0.68")) {
             leg2->AddEntry(x.second["hdose"], "68 #pm 0.68 MeV", "l");
             leg22->AddEntry(x.second["hdose"], "68 #pm 0.68 MeV", "l");
        }
         if(WichCase.Contains("_1")){  leg2->AddEntry(x.second["hdose"], "68 #pm 1.00 MeV", "l");
             leg22->AddEntry(x.second["hdose"], "68 #pm 1.00 MeV", "l");
             }
         if(WichCase.Contains("_0")){  leg2->AddEntry(x.second["hdose"], "68 #pm 0.00 MeV", "l");
             leg22->AddEntry(x.second["hdose"], "68 #pm 0.00 MeV", "l");
             }
     ...
     pt = new TPaveText(0.6558577,0.7168972,0.9063808,0.8621542,"blNDC");
     MaquillageGraphique_PaveText(pt,map_Comment_Ref);
